...
class Command(BaseCommand):
    ...
    def import_data(self, model, file_name, fields, foreign_keys=None):
        ...
        try:
                    ...
                    try:
                        ...
                        if foreign_keys:
                            for field, (fk_model, fk_field) in foreign_keys.items():
                                fk_value = row[field]
                                fk_object = fk_model.objects.filter(**{fk_field: fk_value}).first()
                                actual_field = field[:-3] if field.endswith('_id') else field
                        ...
                        self.stdout.write(self.style.WARNING(f'Ошибка в {file_name}: {e}'))
                if objects:
            ...
            self.stdout.write(self.style.ERROR(f'Ошибка при чтении {file_name}: {e}'))
